...
async def test_results_sorted_by_score_descending():
    ...
    with patch.object(
        retriever._client.embeddings, "create", new=AsyncMock(return_value=_mock_embedding_response([1.0, 0.0]))
    ):
        results = await retriever.retrieve("query", top_k=3, min_score=0.0)
...
async def test_chunks_below_min_score_excluded():
    retriever = _make_retriever(_make_index())
    with patch.object(
        retriever._client.embeddings, "create", new=AsyncMock(return_value=_mock_embedding_response([1.0, 0.0]))
    ):
        results = await retriever.retrieve("query", top_k=3, min_score=0.5)
...
async def test_all_below_threshold_returns_empty_list():
    ...
    with patch.object(
        ...
        results = await retriever.retrieve("query", top_k=3, min_score=1.5)

    assert results == []


async def test_faiss_padding_idx_skipped_when_top_k_exceeds_ntotal():
    retriever = _make_retriever(_make_index())
    with patch.object(
        retriever._client.embeddings, "create", new=AsyncMock(return_value=_mock_embedding_response([1.0, 0.0]))
    ):
        # top_k > ntotal (3) forces FAISS to pad the result with idx == -1
        results = await retriever.retrieve("query", top_k=10, min_score=0.0)
